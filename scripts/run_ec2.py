...
def run_simulation(worker_id: int, cfg: SimConfig):
    ...
    # シミュレーション結果を保存
    results_file_name = f"{worker_id:02d}_results.npz"
    np.savez_compressed(
        results_file_name,
        event_times=times_all, 
        event_nodes=nodes_all
    )
    upload_to_s3(results_file_name, out)
    
    # ネットワーク情報を保存（ネットワークが自動生成された場合のみ）
    if cfg.network is None:
        network_file_name = f"{worker_id:02d}_network.npz"
        np.savez_compressed(
            network_file_name,
            N=g.N, 
            k_ave=g.k_ave,
        ...
        upload_to_s3(network_file_name, out)
...
def save_params(cfg: SimConfig):
    ...
    upload_to_s3(config_file_name, out)

    # 分岐比の値をnumpy配列として保存
    r_frac_file_name = "r_frac_values.npy"
    np.save(r_frac_file_name, cfg.r_frac_values)
    upload_to_s3(r_frac_file_name, out)
